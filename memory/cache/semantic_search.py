...
def search_similar_output(input_text: str, top_k: int = 5, threshold: float = 0.3) -> list:
    try:
        ...
        for score, idx in zip(D[0], I[0]):
            ...
            log_event(f"[DEBUG] idx={idx}, score={score:.4f}, sim={approx_sim:.4f}, threshold={threshold}", "DEBUG")
            if idx >= 0 and approx_sim >= threshold:
                ...
                for line in cache_lines:
                    try:
                        ...
                        log_event(f"[WARN] Failed to parse cache line: {e}", "WARN")
                        continue
